...
if (fullPath.endsWith('/admin')) {
  activeTab.value = "/courses"
} else if (fullPath.endsWith("/tasks")) {
  activeTab.value = "/tasks"
} else if (fullPath.endsWith("/lessons")) {
  activeTab.value = "/admin/lessons"
}
</script>
<template>
  <v-app class="relative min-h-screen">
      ...
      <v-container>
        <v-row>
          ...
          <v-col cols="6" class="hidden md:flex justify-center align-center text-sm">
            ...
            <div class="select-item" @click="setActiveTab('/admin/lessons')" :class="{
              'active-item': activeTab == '/admin/lessons',
              'black-border': activeTab == '/admin/lessons' && theme.global.name.value == 'light',
              'white-border': activeTab == '/admin/lessons' && theme.global.name.value == 'dark',
            }">
              Уроки
            </div>
    ...
    <div v-if="drawer">
      <v-navigation-drawer v-model="drawer" location="right" temporary>
        ...
        <v-divider></v-divider>
        <v-col class='flex justify-start' cols='12'>
          <v-btn @click="router.push('/admin/settings')" class="" rounded="xl" variant="outlined">
            настройки
          </v-btn>
        </v-col>
